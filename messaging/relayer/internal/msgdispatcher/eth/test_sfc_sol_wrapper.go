// Code generated - DO NOT EDIT.
// This file is a generated binding and any manual changes will be lost.

package eth

import (
	"errors"
	"math/big"
	"strings"

	ethereum "github.com/ethereum/go-ethereum"
	"github.com/ethereum/go-ethereum/accounts/abi"
	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/event"
)

// Reference imports to suppress errors if they are not otherwise used.
var (
	_ = errors.New
	_ = big.NewInt
	_ = strings.NewReader
	_ = ethereum.NotFound
	_ = bind.Bind
	_ = common.Big1
	_ = types.BloomLookup
	_ = event.NewSubscription
)

// TestsfcMetaData contains all meta data concerning the Testsfc contract.
var TestsfcMetaData = &bind.MetaData{
	ABI: "[{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"_myBlockchainId\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"_timeHorizon\",\"type\":\"uint256\"}],\"stateMutability\":\"nonpayable\",\"type\":\"constructor\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"string\",\"name\":\"_revertReason\",\"type\":\"string\"}],\"name\":\"CallFailure\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"bytes32\",\"name\":\"_txId\",\"type\":\"bytes32\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"_timestamp\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"address\",\"name\":\"_caller\",\"type\":\"address\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"_destBcId\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"address\",\"name\":\"_destContract\",\"type\":\"address\"},{\"indexed\":false,\"internalType\":\"bytes\",\"name\":\"_destFunctionCall\",\"type\":\"bytes\"}],\"name\":\"CrossCall\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"address\",\"name\":\"previousOwner\",\"type\":\"address\"},{\"indexed\":true,\"internalType\":\"address\",\"name\":\"newOwner\",\"type\":\"address\"}],\"name\":\"OwnershipTransferred\",\"type\":\"event\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"_blockchainId\",\"type\":\"uint256\"},{\"internalType\":\"address\",\"name\":\"_cbc\",\"type\":\"address\"}],\"name\":\"addRemoteCrosschainControl\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"_blockchainId\",\"type\":\"uint256\"},{\"internalType\":\"address\",\"name\":\"_verifier\",\"type\":\"address\"}],\"name\":\"addVerifier\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"_destBcId\",\"type\":\"uint256\"},{\"internalType\":\"address\",\"name\":\"_destContract\",\"type\":\"address\"},{\"internalType\":\"bytes\",\"name\":\"_destData\",\"type\":\"bytes\"}],\"name\":\"crossBlockchainCall\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"_sourceBcId\",\"type\":\"uint256\"},{\"internalType\":\"address\",\"name\":\"_cbcAddress\",\"type\":\"address\"},{\"internalType\":\"bytes\",\"name\":\"_eventData\",\"type\":\"bytes\"},{\"internalType\":\"bytes\",\"name\":\"_signature\",\"type\":\"bytes\"}],\"name\":\"crossCallHandler\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"_sourceBcId\",\"type\":\"uint256\"},{\"internalType\":\"address\",\"name\":\"_cbcAddress\",\"type\":\"address\"},{\"internalType\":\"bytes\",\"name\":\"_eventData\",\"type\":\"bytes\"},{\"internalType\":\"bytes\",\"name\":\"_signature\",\"type\":\"bytes\"},{\"internalType\":\"bytes32[]\",\"name\":\"_oldTxIds\",\"type\":\"bytes32[]\"}],\"name\":\"crossCallHandlerSaveGas\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"myBlockchainId\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"owner\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"renounceOwnership\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"bytes32\",\"name\":\"\",\"type\":\"bytes32\"}],\"name\":\"replayPrevention\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"timeHorizon\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"newOwner\",\"type\":\"address\"}],\"name\":\"transferOwnership\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"}]",
	Bin: "",
}

// TestsfcABI is the input ABI used to generate the binding from.
// Deprecated: Use TestsfcMetaData.ABI instead.
var TestsfcABI = TestsfcMetaData.ABI

// TestsfcBin is the compiled bytecode used for deploying new contracts.
// Deprecated: Use TestsfcMetaData.Bin instead.
var TestsfcBin = TestsfcMetaData.Bin

// DeployTestsfc deploys a new Ethereum contract, binding an instance of Testsfc to it.
func DeployTestsfc(auth *bind.TransactOpts, backend bind.ContractBackend, _myBlockchainId *big.Int, _timeHorizon *big.Int) (common.Address, *types.Transaction, *Testsfc, error) {
	parsed, err := TestsfcMetaData.GetAbi()
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	if parsed == nil {
		return common.Address{}, nil, nil, errors.New("GetABI returned nil")
	}

	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(TestsfcBin), backend, _myBlockchainId, _timeHorizon)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	return address, tx, &Testsfc{TestsfcCaller: TestsfcCaller{contract: contract}, TestsfcTransactor: TestsfcTransactor{contract: contract}, TestsfcFilterer: TestsfcFilterer{contract: contract}}, nil
}

// Testsfc is an auto generated Go binding around an Ethereum contract.
type Testsfc struct {
	TestsfcCaller     // Read-only binding to the contract
	TestsfcTransactor // Write-only binding to the contract
	TestsfcFilterer   // Log filterer for contract events
}

// TestsfcCaller is an auto generated read-only Go binding around an Ethereum contract.
type TestsfcCaller struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// TestsfcTransactor is an auto generated write-only Go binding around an Ethereum contract.
type TestsfcTransactor struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// TestsfcFilterer is an auto generated log filtering Go binding around an Ethereum contract events.
type TestsfcFilterer struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// TestsfcSession is an auto generated Go binding around an Ethereum contract,
// with pre-set call and transact options.
type TestsfcSession struct {
	Contract     *Testsfc          // Generic contract binding to set the session for
	CallOpts     bind.CallOpts     // Call options to use throughout this session
	TransactOpts bind.TransactOpts // Transaction auth options to use throughout this session
}

// TestsfcCallerSession is an auto generated read-only Go binding around an Ethereum contract,
// with pre-set call options.
type TestsfcCallerSession struct {
	Contract *TestsfcCaller // Generic contract caller binding to set the session for
	CallOpts bind.CallOpts  // Call options to use throughout this session
}

// TestsfcTransactorSession is an auto generated write-only Go binding around an Ethereum contract,
// with pre-set transact options.
type TestsfcTransactorSession struct {
	Contract     *TestsfcTransactor // Generic contract transactor binding to set the session for
	TransactOpts bind.TransactOpts  // Transaction auth options to use throughout this session
}

// TestsfcRaw is an auto generated low-level Go binding around an Ethereum contract.
type TestsfcRaw struct {
	Contract *Testsfc // Generic contract binding to access the raw methods on
}

// TestsfcCallerRaw is an auto generated low-level read-only Go binding around an Ethereum contract.
type TestsfcCallerRaw struct {
	Contract *TestsfcCaller // Generic read-only contract binding to access the raw methods on
}

// TestsfcTransactorRaw is an auto generated low-level write-only Go binding around an Ethereum contract.
type TestsfcTransactorRaw struct {
	Contract *TestsfcTransactor // Generic write-only contract binding to access the raw methods on
}

// NewTestsfc creates a new instance of Testsfc, bound to a specific deployed contract.
func NewTestsfc(address common.Address, backend bind.ContractBackend) (*Testsfc, error) {
	contract, err := bindTestsfc(address, backend, backend, backend)
	if err != nil {
		return nil, err
	}
	return &Testsfc{TestsfcCaller: TestsfcCaller{contract: contract}, TestsfcTransactor: TestsfcTransactor{contract: contract}, TestsfcFilterer: TestsfcFilterer{contract: contract}}, nil
}

// NewTestsfcCaller creates a new read-only instance of Testsfc, bound to a specific deployed contract.
func NewTestsfcCaller(address common.Address, caller bind.ContractCaller) (*TestsfcCaller, error) {
	contract, err := bindTestsfc(address, caller, nil, nil)
	if err != nil {
		return nil, err
	}
	return &TestsfcCaller{contract: contract}, nil
}

// NewTestsfcTransactor creates a new write-only instance of Testsfc, bound to a specific deployed contract.
func NewTestsfcTransactor(address common.Address, transactor bind.ContractTransactor) (*TestsfcTransactor, error) {
	contract, err := bindTestsfc(address, nil, transactor, nil)
	if err != nil {
		return nil, err
	}
	return &TestsfcTransactor{contract: contract}, nil
}

// NewTestsfcFilterer creates a new log filterer instance of Testsfc, bound to a specific deployed contract.
func NewTestsfcFilterer(address common.Address, filterer bind.ContractFilterer) (*TestsfcFilterer, error) {
	contract, err := bindTestsfc(address, nil, nil, filterer)
	if err != nil {
		return nil, err
	}
	return &TestsfcFilterer{contract: contract}, nil
}

// bindTestsfc binds a generic wrapper to an already deployed contract.
func bindTestsfc(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := abi.JSON(strings.NewReader(TestsfcABI))
	if err != nil {
		return nil, err
	}
	return bind.NewBoundContract(address, parsed, caller, transactor, filterer), nil
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_Testsfc *TestsfcRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _Testsfc.Contract.TestsfcCaller.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_Testsfc *TestsfcRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Testsfc.Contract.TestsfcTransactor.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_Testsfc *TestsfcRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _Testsfc.Contract.TestsfcTransactor.contract.Transact(opts, method, params...)
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_Testsfc *TestsfcCallerRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _Testsfc.Contract.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_Testsfc *TestsfcTransactorRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Testsfc.Contract.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_Testsfc *TestsfcTransactorRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _Testsfc.Contract.contract.Transact(opts, method, params...)
}

// MyBlockchainId is a free data retrieval call binding the contract method 0x439160df.
//
// Solidity: function myBlockchainId() view returns(uint256)
func (_Testsfc *TestsfcCaller) MyBlockchainId(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _Testsfc.contract.Call(opts, &out, "myBlockchainId")

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// MyBlockchainId is a free data retrieval call binding the contract method 0x439160df.
//
// Solidity: function myBlockchainId() view returns(uint256)
func (_Testsfc *TestsfcSession) MyBlockchainId() (*big.Int, error) {
	return _Testsfc.Contract.MyBlockchainId(&_Testsfc.CallOpts)
}

// MyBlockchainId is a free data retrieval call binding the contract method 0x439160df.
//
// Solidity: function myBlockchainId() view returns(uint256)
func (_Testsfc *TestsfcCallerSession) MyBlockchainId() (*big.Int, error) {
	return _Testsfc.Contract.MyBlockchainId(&_Testsfc.CallOpts)
}

// Owner is a free data retrieval call binding the contract method 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (_Testsfc *TestsfcCaller) Owner(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Testsfc.contract.Call(opts, &out, "owner")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// Owner is a free data retrieval call binding the contract method 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (_Testsfc *TestsfcSession) Owner() (common.Address, error) {
	return _Testsfc.Contract.Owner(&_Testsfc.CallOpts)
}

// Owner is a free data retrieval call binding the contract method 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (_Testsfc *TestsfcCallerSession) Owner() (common.Address, error) {
	return _Testsfc.Contract.Owner(&_Testsfc.CallOpts)
}

// ReplayPrevention is a free data retrieval call binding the contract method 0x0a3ef1f2.
//
// Solidity: function replayPrevention(bytes32 ) view returns(uint256)
func (_Testsfc *TestsfcCaller) ReplayPrevention(opts *bind.CallOpts, arg0 [32]byte) (*big.Int, error) {
	var out []interface{}
	err := _Testsfc.contract.Call(opts, &out, "replayPrevention", arg0)

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// ReplayPrevention is a free data retrieval call binding the contract method 0x0a3ef1f2.
//
// Solidity: function replayPrevention(bytes32 ) view returns(uint256)
func (_Testsfc *TestsfcSession) ReplayPrevention(arg0 [32]byte) (*big.Int, error) {
	return _Testsfc.Contract.ReplayPrevention(&_Testsfc.CallOpts, arg0)
}

// ReplayPrevention is a free data retrieval call binding the contract method 0x0a3ef1f2.
//
// Solidity: function replayPrevention(bytes32 ) view returns(uint256)
func (_Testsfc *TestsfcCallerSession) ReplayPrevention(arg0 [32]byte) (*big.Int, error) {
	return _Testsfc.Contract.ReplayPrevention(&_Testsfc.CallOpts, arg0)
}

// TimeHorizon is a free data retrieval call binding the contract method 0x1101b8f0.
//
// Solidity: function timeHorizon() view returns(uint256)
func (_Testsfc *TestsfcCaller) TimeHorizon(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _Testsfc.contract.Call(opts, &out, "timeHorizon")

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// TimeHorizon is a free data retrieval call binding the contract method 0x1101b8f0.
//
// Solidity: function timeHorizon() view returns(uint256)
func (_Testsfc *TestsfcSession) TimeHorizon() (*big.Int, error) {
	return _Testsfc.Contract.TimeHorizon(&_Testsfc.CallOpts)
}

// TimeHorizon is a free data retrieval call binding the contract method 0x1101b8f0.
//
// Solidity: function timeHorizon() view returns(uint256)
func (_Testsfc *TestsfcCallerSession) TimeHorizon() (*big.Int, error) {
	return _Testsfc.Contract.TimeHorizon(&_Testsfc.CallOpts)
}

// AddRemoteCrosschainControl is a paid mutator transaction binding the contract method 0x19836dc7.
//
// Solidity: function addRemoteCrosschainControl(uint256 _blockchainId, address _cbc) returns()
func (_Testsfc *TestsfcTransactor) AddRemoteCrosschainControl(opts *bind.TransactOpts, _blockchainId *big.Int, _cbc common.Address) (*types.Transaction, error) {
	return _Testsfc.contract.Transact(opts, "addRemoteCrosschainControl", _blockchainId, _cbc)
}

// AddRemoteCrosschainControl is a paid mutator transaction binding the contract method 0x19836dc7.
//
// Solidity: function addRemoteCrosschainControl(uint256 _blockchainId, address _cbc) returns()
func (_Testsfc *TestsfcSession) AddRemoteCrosschainControl(_blockchainId *big.Int, _cbc common.Address) (*types.Transaction, error) {
	return _Testsfc.Contract.AddRemoteCrosschainControl(&_Testsfc.TransactOpts, _blockchainId, _cbc)
}

// AddRemoteCrosschainControl is a paid mutator transaction binding the contract method 0x19836dc7.
//
// Solidity: function addRemoteCrosschainControl(uint256 _blockchainId, address _cbc) returns()
func (_Testsfc *TestsfcTransactorSession) AddRemoteCrosschainControl(_blockchainId *big.Int, _cbc common.Address) (*types.Transaction, error) {
	return _Testsfc.Contract.AddRemoteCrosschainControl(&_Testsfc.TransactOpts, _blockchainId, _cbc)
}

// AddVerifier is a paid mutator transaction binding the contract method 0xb2832096.
//
// Solidity: function addVerifier(uint256 _blockchainId, address _verifier) returns()
func (_Testsfc *TestsfcTransactor) AddVerifier(opts *bind.TransactOpts, _blockchainId *big.Int, _verifier common.Address) (*types.Transaction, error) {
	return _Testsfc.contract.Transact(opts, "addVerifier", _blockchainId, _verifier)
}

// AddVerifier is a paid mutator transaction binding the contract method 0xb2832096.
//
// Solidity: function addVerifier(uint256 _blockchainId, address _verifier) returns()
func (_Testsfc *TestsfcSession) AddVerifier(_blockchainId *big.Int, _verifier common.Address) (*types.Transaction, error) {
	return _Testsfc.Contract.AddVerifier(&_Testsfc.TransactOpts, _blockchainId, _verifier)
}

// AddVerifier is a paid mutator transaction binding the contract method 0xb2832096.
//
// Solidity: function addVerifier(uint256 _blockchainId, address _verifier) returns()
func (_Testsfc *TestsfcTransactorSession) AddVerifier(_blockchainId *big.Int, _verifier common.Address) (*types.Transaction, error) {
	return _Testsfc.Contract.AddVerifier(&_Testsfc.TransactOpts, _blockchainId, _verifier)
}

// CrossBlockchainCall is a paid mutator transaction binding the contract method 0x92b2c335.
//
// Solidity: function crossBlockchainCall(uint256 _destBcId, address _destContract, bytes _destData) returns()
func (_Testsfc *TestsfcTransactor) CrossBlockchainCall(opts *bind.TransactOpts, _destBcId *big.Int, _destContract common.Address, _destData []byte) (*types.Transaction, error) {
	return _Testsfc.contract.Transact(opts, "crossBlockchainCall", _destBcId, _destContract, _destData)
}

// CrossBlockchainCall is a paid mutator transaction binding the contract method 0x92b2c335.
//
// Solidity: function crossBlockchainCall(uint256 _destBcId, address _destContract, bytes _destData) returns()
func (_Testsfc *TestsfcSession) CrossBlockchainCall(_destBcId *big.Int, _destContract common.Address, _destData []byte) (*types.Transaction, error) {
	return _Testsfc.Contract.CrossBlockchainCall(&_Testsfc.TransactOpts, _destBcId, _destContract, _destData)
}

// CrossBlockchainCall is a paid mutator transaction binding the contract method 0x92b2c335.
//
// Solidity: function crossBlockchainCall(uint256 _destBcId, address _destContract, bytes _destData) returns()
func (_Testsfc *TestsfcTransactorSession) CrossBlockchainCall(_destBcId *big.Int, _destContract common.Address, _destData []byte) (*types.Transaction, error) {
	return _Testsfc.Contract.CrossBlockchainCall(&_Testsfc.TransactOpts, _destBcId, _destContract, _destData)
}

// CrossCallHandler is a paid mutator transaction binding the contract method 0x40884052.
//
// Solidity: function crossCallHandler(uint256 _sourceBcId, address _cbcAddress, bytes _eventData, bytes _signature) returns()
func (_Testsfc *TestsfcTransactor) CrossCallHandler(opts *bind.TransactOpts, _sourceBcId *big.Int, _cbcAddress common.Address, _eventData []byte, _signature []byte) (*types.Transaction, error) {
	return _Testsfc.contract.Transact(opts, "crossCallHandler", _sourceBcId, _cbcAddress, _eventData, _signature)
}

// CrossCallHandler is a paid mutator transaction binding the contract method 0x40884052.
//
// Solidity: function crossCallHandler(uint256 _sourceBcId, address _cbcAddress, bytes _eventData, bytes _signature) returns()
func (_Testsfc *TestsfcSession) CrossCallHandler(_sourceBcId *big.Int, _cbcAddress common.Address, _eventData []byte, _signature []byte) (*types.Transaction, error) {
	return _Testsfc.Contract.CrossCallHandler(&_Testsfc.TransactOpts, _sourceBcId, _cbcAddress, _eventData, _signature)
}

// CrossCallHandler is a paid mutator transaction binding the contract method 0x40884052.
//
// Solidity: function crossCallHandler(uint256 _sourceBcId, address _cbcAddress, bytes _eventData, bytes _signature) returns()
func (_Testsfc *TestsfcTransactorSession) CrossCallHandler(_sourceBcId *big.Int, _cbcAddress common.Address, _eventData []byte, _signature []byte) (*types.Transaction, error) {
	return _Testsfc.Contract.CrossCallHandler(&_Testsfc.TransactOpts, _sourceBcId, _cbcAddress, _eventData, _signature)
}

// CrossCallHandlerSaveGas is a paid mutator transaction binding the contract method 0xf51a72d8.
//
// Solidity: function crossCallHandlerSaveGas(uint256 _sourceBcId, address _cbcAddress, bytes _eventData, bytes _signature, bytes32[] _oldTxIds) returns()
func (_Testsfc *TestsfcTransactor) CrossCallHandlerSaveGas(opts *bind.TransactOpts, _sourceBcId *big.Int, _cbcAddress common.Address, _eventData []byte, _signature []byte, _oldTxIds [][32]byte) (*types.Transaction, error) {
	return _Testsfc.contract.Transact(opts, "crossCallHandlerSaveGas", _sourceBcId, _cbcAddress, _eventData, _signature, _oldTxIds)
}

// CrossCallHandlerSaveGas is a paid mutator transaction binding the contract method 0xf51a72d8.
//
// Solidity: function crossCallHandlerSaveGas(uint256 _sourceBcId, address _cbcAddress, bytes _eventData, bytes _signature, bytes32[] _oldTxIds) returns()
func (_Testsfc *TestsfcSession) CrossCallHandlerSaveGas(_sourceBcId *big.Int, _cbcAddress common.Address, _eventData []byte, _signature []byte, _oldTxIds [][32]byte) (*types.Transaction, error) {
	return _Testsfc.Contract.CrossCallHandlerSaveGas(&_Testsfc.TransactOpts, _sourceBcId, _cbcAddress, _eventData, _signature, _oldTxIds)
}

// CrossCallHandlerSaveGas is a paid mutator transaction binding the contract method 0xf51a72d8.
//
// Solidity: function crossCallHandlerSaveGas(uint256 _sourceBcId, address _cbcAddress, bytes _eventData, bytes _signature, bytes32[] _oldTxIds) returns()
func (_Testsfc *TestsfcTransactorSession) CrossCallHandlerSaveGas(_sourceBcId *big.Int, _cbcAddress common.Address, _eventData []byte, _signature []byte, _oldTxIds [][32]byte) (*types.Transaction, error) {
	return _Testsfc.Contract.CrossCallHandlerSaveGas(&_Testsfc.TransactOpts, _sourceBcId, _cbcAddress, _eventData, _signature, _oldTxIds)
}

// RenounceOwnership is a paid mutator transaction binding the contract method 0x715018a6.
//
// Solidity: function renounceOwnership() returns()
func (_Testsfc *TestsfcTransactor) RenounceOwnership(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Testsfc.contract.Transact(opts, "renounceOwnership")
}

// RenounceOwnership is a paid mutator transaction binding the contract method 0x715018a6.
//
// Solidity: function renounceOwnership() returns()
func (_Testsfc *TestsfcSession) RenounceOwnership() (*types.Transaction, error) {
	return _Testsfc.Contract.RenounceOwnership(&_Testsfc.TransactOpts)
}

// RenounceOwnership is a paid mutator transaction binding the contract method 0x715018a6.
//
// Solidity: function renounceOwnership() returns()
func (_Testsfc *TestsfcTransactorSession) RenounceOwnership() (*types.Transaction, error) {
	return _Testsfc.Contract.RenounceOwnership(&_Testsfc.TransactOpts)
}

// TransferOwnership is a paid mutator transaction binding the contract method 0xf2fde38b.
//
// Solidity: function transferOwnership(address newOwner) returns()
func (_Testsfc *TestsfcTransactor) TransferOwnership(opts *bind.TransactOpts, newOwner common.Address) (*types.Transaction, error) {
	return _Testsfc.contract.Transact(opts, "transferOwnership", newOwner)
}

// TransferOwnership is a paid mutator transaction binding the contract method 0xf2fde38b.
//
// Solidity: function transferOwnership(address newOwner) returns()
func (_Testsfc *TestsfcSession) TransferOwnership(newOwner common.Address) (*types.Transaction, error) {
	return _Testsfc.Contract.TransferOwnership(&_Testsfc.TransactOpts, newOwner)
}

// TransferOwnership is a paid mutator transaction binding the contract method 0xf2fde38b.
//
// Solidity: function transferOwnership(address newOwner) returns()
func (_Testsfc *TestsfcTransactorSession) TransferOwnership(newOwner common.Address) (*types.Transaction, error) {
	return _Testsfc.Contract.TransferOwnership(&_Testsfc.TransactOpts, newOwner)
}

// TestsfcCallFailureIterator is returned from FilterCallFailure and is used to iterate over the raw logs and unpacked data for CallFailure events raised by the Testsfc contract.
type TestsfcCallFailureIterator struct {
	Event *TestsfcCallFailure // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *TestsfcCallFailureIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(TestsfcCallFailure)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(TestsfcCallFailure)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *TestsfcCallFailureIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *TestsfcCallFailureIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// TestsfcCallFailure represents a CallFailure event raised by the Testsfc contract.
type TestsfcCallFailure struct {
	RevertReason string
	Raw          types.Log // Blockchain specific contextual infos
}

// FilterCallFailure is a free log retrieval operation binding the contract event 0x38e7ccc4b02b2da681f96e62aef89b5c6d4115f501f8d42430bb2f5f2fa981a6.
//
// Solidity: event CallFailure(string _revertReason)
func (_Testsfc *TestsfcFilterer) FilterCallFailure(opts *bind.FilterOpts) (*TestsfcCallFailureIterator, error) {

	logs, sub, err := _Testsfc.contract.FilterLogs(opts, "CallFailure")
	if err != nil {
		return nil, err
	}
	return &TestsfcCallFailureIterator{contract: _Testsfc.contract, event: "CallFailure", logs: logs, sub: sub}, nil
}

// WatchCallFailure is a free log subscription operation binding the contract event 0x38e7ccc4b02b2da681f96e62aef89b5c6d4115f501f8d42430bb2f5f2fa981a6.
//
// Solidity: event CallFailure(string _revertReason)
func (_Testsfc *TestsfcFilterer) WatchCallFailure(opts *bind.WatchOpts, sink chan<- *TestsfcCallFailure) (event.Subscription, error) {

	logs, sub, err := _Testsfc.contract.WatchLogs(opts, "CallFailure")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(TestsfcCallFailure)
				if err := _Testsfc.contract.UnpackLog(event, "CallFailure", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseCallFailure is a log parse operation binding the contract event 0x38e7ccc4b02b2da681f96e62aef89b5c6d4115f501f8d42430bb2f5f2fa981a6.
//
// Solidity: event CallFailure(string _revertReason)
func (_Testsfc *TestsfcFilterer) ParseCallFailure(log types.Log) (*TestsfcCallFailure, error) {
	event := new(TestsfcCallFailure)
	if err := _Testsfc.contract.UnpackLog(event, "CallFailure", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// TestsfcCrossCallIterator is returned from FilterCrossCall and is used to iterate over the raw logs and unpacked data for CrossCall events raised by the Testsfc contract.
type TestsfcCrossCallIterator struct {
	Event *TestsfcCrossCall // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *TestsfcCrossCallIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(TestsfcCrossCall)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(TestsfcCrossCall)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *TestsfcCrossCallIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *TestsfcCrossCallIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// TestsfcCrossCall represents a CrossCall event raised by the Testsfc contract.
type TestsfcCrossCall struct {
	TxId             [32]byte
	Timestamp        *big.Int
	Caller           common.Address
	DestBcId         *big.Int
	DestContract     common.Address
	DestFunctionCall []byte
	Raw              types.Log // Blockchain specific contextual infos
}

// FilterCrossCall is a free log retrieval operation binding the contract event 0x59f736dc5e15c4b12526487502645403b0a4316d82eba7e9ecdc2a050c10ad27.
//
// Solidity: event CrossCall(bytes32 _txId, uint256 _timestamp, address _caller, uint256 _destBcId, address _destContract, bytes _destFunctionCall)
func (_Testsfc *TestsfcFilterer) FilterCrossCall(opts *bind.FilterOpts) (*TestsfcCrossCallIterator, error) {

	logs, sub, err := _Testsfc.contract.FilterLogs(opts, "CrossCall")
	if err != nil {
		return nil, err
	}
	return &TestsfcCrossCallIterator{contract: _Testsfc.contract, event: "CrossCall", logs: logs, sub: sub}, nil
}

// WatchCrossCall is a free log subscription operation binding the contract event 0x59f736dc5e15c4b12526487502645403b0a4316d82eba7e9ecdc2a050c10ad27.
//
// Solidity: event CrossCall(bytes32 _txId, uint256 _timestamp, address _caller, uint256 _destBcId, address _destContract, bytes _destFunctionCall)
func (_Testsfc *TestsfcFilterer) WatchCrossCall(opts *bind.WatchOpts, sink chan<- *TestsfcCrossCall) (event.Subscription, error) {

	logs, sub, err := _Testsfc.contract.WatchLogs(opts, "CrossCall")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(TestsfcCrossCall)
				if err := _Testsfc.contract.UnpackLog(event, "CrossCall", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseCrossCall is a log parse operation binding the contract event 0x59f736dc5e15c4b12526487502645403b0a4316d82eba7e9ecdc2a050c10ad27.
//
// Solidity: event CrossCall(bytes32 _txId, uint256 _timestamp, address _caller, uint256 _destBcId, address _destContract, bytes _destFunctionCall)
func (_Testsfc *TestsfcFilterer) ParseCrossCall(log types.Log) (*TestsfcCrossCall, error) {
	event := new(TestsfcCrossCall)
	if err := _Testsfc.contract.UnpackLog(event, "CrossCall", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// TestsfcOwnershipTransferredIterator is returned from FilterOwnershipTransferred and is used to iterate over the raw logs and unpacked data for OwnershipTransferred events raised by the Testsfc contract.
type TestsfcOwnershipTransferredIterator struct {
	Event *TestsfcOwnershipTransferred // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *TestsfcOwnershipTransferredIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(TestsfcOwnershipTransferred)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(TestsfcOwnershipTransferred)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *TestsfcOwnershipTransferredIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *TestsfcOwnershipTransferredIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// TestsfcOwnershipTransferred represents a OwnershipTransferred event raised by the Testsfc contract.
type TestsfcOwnershipTransferred struct {
	PreviousOwner common.Address
	NewOwner      common.Address
	Raw           types.Log // Blockchain specific contextual infos
}

// FilterOwnershipTransferred is a free log retrieval operation binding the contract event 0x8be0079c531659141344cd1fd0a4f28419497f9722a3daafe3b4186f6b6457e0.
//
// Solidity: event OwnershipTransferred(address indexed previousOwner, address indexed newOwner)
func (_Testsfc *TestsfcFilterer) FilterOwnershipTransferred(opts *bind.FilterOpts, previousOwner []common.Address, newOwner []common.Address) (*TestsfcOwnershipTransferredIterator, error) {

	var previousOwnerRule []interface{}
	for _, previousOwnerItem := range previousOwner {
		previousOwnerRule = append(previousOwnerRule, previousOwnerItem)
	}
	var newOwnerRule []interface{}
	for _, newOwnerItem := range newOwner {
		newOwnerRule = append(newOwnerRule, newOwnerItem)
	}

	logs, sub, err := _Testsfc.contract.FilterLogs(opts, "OwnershipTransferred", previousOwnerRule, newOwnerRule)
	if err != nil {
		return nil, err
	}
	return &TestsfcOwnershipTransferredIterator{contract: _Testsfc.contract, event: "OwnershipTransferred", logs: logs, sub: sub}, nil
}

// WatchOwnershipTransferred is a free log subscription operation binding the contract event 0x8be0079c531659141344cd1fd0a4f28419497f9722a3daafe3b4186f6b6457e0.
//
// Solidity: event OwnershipTransferred(address indexed previousOwner, address indexed newOwner)
func (_Testsfc *TestsfcFilterer) WatchOwnershipTransferred(opts *bind.WatchOpts, sink chan<- *TestsfcOwnershipTransferred, previousOwner []common.Address, newOwner []common.Address) (event.Subscription, error) {

	var previousOwnerRule []interface{}
	for _, previousOwnerItem := range previousOwner {
		previousOwnerRule = append(previousOwnerRule, previousOwnerItem)
	}
	var newOwnerRule []interface{}
	for _, newOwnerItem := range newOwner {
		newOwnerRule = append(newOwnerRule, newOwnerItem)
	}

	logs, sub, err := _Testsfc.contract.WatchLogs(opts, "OwnershipTransferred", previousOwnerRule, newOwnerRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(TestsfcOwnershipTransferred)
				if err := _Testsfc.contract.UnpackLog(event, "OwnershipTransferred", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseOwnershipTransferred is a log parse operation binding the contract event 0x8be0079c531659141344cd1fd0a4f28419497f9722a3daafe3b4186f6b6457e0.
//
// Solidity: event OwnershipTransferred(address indexed previousOwner, address indexed newOwner)
func (_Testsfc *TestsfcFilterer) ParseOwnershipTransferred(log types.Log) (*TestsfcOwnershipTransferred, error) {
	event := new(TestsfcOwnershipTransferred)
	if err := _Testsfc.contract.UnpackLog(event, "OwnershipTransferred", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}
