...
import (
	...
	"github.com/consensys/gpact/messaging/relayer/internal/soliditywrappers/test"
...
func TestFreeConsortiumTransaciton(t *testing.T) {
	...
	testRegistrarAddress, testRegistrarTxReceipt, registrarInstance, err := registrar.DeployRegistrar(auth, client)
	assert.Nil(t, err)
	...
	testSfcAddress, testSfcTxReceipt, _ /*testSfcInstance */, err := sfc.DeploySfc(auth, client, blockchainId, sfcTimeHorizon)
	assert.Nil(t, err)
	...
	eventStoreAddress, eventStoreTxReceipt, _ /* eventStoreInstance */, err := eventstore.DeployEventstore(auth, client, testRegistrarAddress, testSfcAddress)
	assert.Nil(t, err)
	...
	testMsgDispatcherAddress, testMsgDispatcherTxReceipt, _ /*testMsgDispatcherInstance*/, err := test.DeployTest(auth, client)
	assert.Nil(t, err)
	...
	// TODO See https://github.com/ConsenSys/gpact/blob/main/messaging/event-relay/src/test/java/net/consensys/gpact/eventrelay/EventRelayTest.java
	// to work out what should happen from here.
