...
type MsgDispatcher struct {
	...
	eventRelay        *EventRelayMsgDispatcher
...
func NewMsgDispatcher(c *MsgDispatcherConfig) (*MsgDispatcher, error) {
	...
	m.eventRelay = NewMEventRelayMsgDispatcher(&m)
...
func (m *MsgDispatcher) Connect() error {
	...
	m.eventRelay.Connect()

	return nil
}

func (m *MsgDispatcher) RelaySignedEvent(
	sourceBcId big.Int,
	cbcAddress gethcommon.Address,
	eventData []byte,
	encodedSignatureOrProof []byte) error {

	// TODO: Switch depending on configuration.
	return m.eventRelay.RelaySignedEvent(sourceBcId, cbcAddress, eventData, encodedSignatureOrProof)
}
